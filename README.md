...
#  提示
如果web服务器没有ssl，那么只需要把https改成http即可。“绑定域名”记得替换！。自己改的脚本，只是把下载源改成了阿里地址：[http://mirrors.ustc.edu.cn/](https://mirrors.aliyun.com/docker-ce)  （只测试过debian和Ubuntu系统，其他系统可能装不上）（原来是中科大，不可用了，改成阿里了
